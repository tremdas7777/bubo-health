            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {(editProduct.images || []).map((url, i) => (<div key={i} className="flex items-center gap-1.5"><div className="flex flex-col gap-0.5 shrink-0"><button disabled={i === 0} onClick={() => { const imgs = [...(editProduct.images || [])]; [imgs[i - 1], imgs[i]] = [imgs[i], imgs[i - 1]]; setEditProduct(p => ({ ...p, images: imgs, image_url: imgs[0] || '' })); }} className="p-0.5 rounded hover:bg-muted disabled:opacity-20 text-muted-foreground" title="Mover para cima">▲</button><button disabled={i === (editProduct.images || []).length - 1} onClick={() => { const imgs = [...(editProduct.images || [])]; [imgs[i], imgs[i + 1]] = [imgs[i + 1], imgs[i]]; setEditProduct(p => ({ ...p, images: imgs, image_url: imgs[0] || '' })); }} className="p-0.5 rounded hover:bg-muted disabled:opacity-20 text-muted-foreground" title="Mover para baixo">▼</button></div><div className="w-10 h-10 bg-muted rounded overflow-hidden shrink-0 relative">{i === 0 && <span className="absolute top-0 left-0 bg-primary text-primary-foreground text-[8px] font-bold px-1 rounded-br z-10">1ª</span>}<img src={url} alt="" className="w-full h-full object-cover" onError={e => (e.currentTarget.style.display = 'none')} /></div><Input value={url} onChange={e => { const imgs = [...(editProduct.images || [])]; imgs[i] = e.target.value; setEditProduct(p => ({ ...p, images: imgs, image_url: imgs[0] || '' })); }} className="text-xs font-mono flex-1" /><button onClick={() => { const imgs = (editProduct.images || []).filter((_, j) => j !== i); setEditProduct(p => ({ ...p, images: imgs, image_url: imgs[0] || '' })); }} className="p-1 text-destructive"><X size={14} /></button></div>))}
