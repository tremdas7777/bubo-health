...
export default function WhatsAppButton() {
  ...
  return (
    ...
    >
      ...
      <span className="absolute inset-0 rounded-full bg-[#25D366] opacity-40 animate-pulse" />

      <span className="relative flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-[#25D366] to-[#128C7E] text-white shadow-[0_8px_24px_rgba(37,211,102,0.45)] ring-4 ring-white/30 transition-all duration-300 group-hover:scale-110 group-hover:shadow-[0_12px_32px_rgba(37,211,102,0.6)]">
        ...
        <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-red-500 rounded-full border-2 border-white text-[9px] font-bold flex items-center justify-center text-white">
          1
        </span>
      </span>

      <span className="absolute right-full top-1/2 -translate-y-1/2 mr-3 whitespace-nowrap bg-foreground text-background text-xs font-medium px-3 py-1.5 rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
        {t("whatsapp.tooltip")}
      </span>
    </a>
  );
