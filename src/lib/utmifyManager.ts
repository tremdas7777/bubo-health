...
export async function testUtmifyToken(token: string): Promise<{ success: boolean; message: string }> {
  ...
  try {
    const response = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        orderId: `test_${Date.now()}`, platform: 'bubohealth-store', paymentMethod: 'pix', status: 'paid',
        ...
        products: [{ id: 'test-product', name: 'Teste Bubo Health', planId: null, planName: null, quantity: 1, priceInCents: 100 }],
